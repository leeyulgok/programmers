...
function solution(cacheSize, cities) {
  const citiesLower = cities.map((city) => city.toLowerCase());
  const HIT = 1;
  const MISS = 5;

  let cache = [];
  let answer = 0;
  for (let i = 0; i < citiesLower.length; i++) {
    const cityIndex = cache.indexOf(citiesLower[i]);
    if (cityIndex > -1) {
      answer += HIT;
