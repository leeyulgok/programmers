...
// 코니가 가진 의상들이 담긴 2차원 배열 costume이 주어질 때 서로 다른 옷의 조합의 수를 return 하도록 solution 함수를 작성해주세요.

function solution(costume) {
  let costumeType = {};

  for(let i = 0; i < costume.length; i++) {
    if(!costumeType[costume[i][1]]) {
      costumeType[costume[i][1]] = 1;
    } else {
      costumeType[costume[i][1]] += 1;
    }
  }
  ...
  let answer = 1;
  for(const type in costumeType) {
    answer *= costumeType[type] + 1;
  }
